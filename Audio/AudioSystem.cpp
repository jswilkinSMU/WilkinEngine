...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, Vec3 const& soundPosition, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	...
	if (channelAssignedToSound)
	{
		...
		FMOD_VECTOR fmodPos = SwizzleFmodtoGame(soundPosition);
...
{
	FMOD::Channel* channelPlayingSound = (FMOD::Channel*)(soundPlaybackID);
	FMOD_VECTOR fmodPos = SwizzleFmodtoGame(soundPosition);
	FMOD_VECTOR fmodVel = { 0.f, 0.f, 0.f };
	channelPlayingSound->set3DAttributes(&fmodPos, &fmodVel);
}
...
{
	FMOD_VECTOR fmodPos = SwizzleFmodtoGame(listenerPosition);
	FMOD_VECTOR fmodFwd = SwizzleFmodtoGame(listenerForward);
	FMOD_VECTOR fmodUp  = SwizzleFmodtoGame(listenerUp);
	FMOD_VECTOR fmodVel = { 0.f, 0.f, 0.f };
...
{
	FMOD::Channel* channelPlayingSound = (FMOD::Channel*)(soundPlaybackID);
	channelPlayingSound->isPlaying(&m_isPlaying);
	return m_isPlaying;
}
...
FMOD_VECTOR const AudioSystem::SwizzleFmodtoGame(Vec3 const& gamePos)
{
	FMOD_VECTOR fmodPos;
